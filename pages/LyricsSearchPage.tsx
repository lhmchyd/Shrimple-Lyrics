...
const LucideIcons = {
  ...
  Music2: (props: React.SVGProps<SVGSVGElement>) => ( // Used for Song Title
  ...
  Languages: (props: React.SVGProps<SVGSVGElement>) => ( // Icon for Lyrics section
  ...
  Link: (props: React.SVGProps<SVGSVGElement>) => ( // Icon for Sources
  ...
  Download: (props: React.SVGProps<SVGSVGElement>) => ( // Icon for Download
...
const LyricsSearchPage: React.FC<LyricsSearchPageProps> = ({
  ...
  const handleDownloadLyrics = () => {
    ...
    if (hasActualEnglishLyrics && hasActualRomanizedLyrics) { // Tabs are shown
    ...
    } else if (hasActualEnglishLyrics) { // Only English (or fallback message)
      lyricsToDownload = searchResult.englishLyrics;
      languageLabel = 'English';
    } else if (hasActualRomanizedLyrics) { // Only Romanized
      lyricsToDownload = searchResult.romanizedLyrics;
      languageLabel = 'Romanized';
    } else {
      return; // No actual lyrics to download
    }

    if (!lyricsToDownload || lyricsToDownload.trim() === "" || lyricsToDownload === "English lyrics not available or not found.") {
        // Do not download placeholder text
        return;
    }
    ...
    <div className="p-4 sm:p-6 space-y-6 h-full">
      {isLoading && (
        <div className="flex justify-center items-center h-full">
          <LoadingSpinner text={`Searching for "${currentQuery || 'lyrics'}"...`} size="lg" />
        </div>
      )}
      ...
      {!isLoading && !error && searchResult && (
        <div className="space-y-6 animate-fadeIn">
          ...
          {/* Song Title & Description Card */}
          {(isSongTitleAvailable || isSongDescriptionAvailable) && (
            <Card className="border-0 shadow-lg">
              {isSongTitleAvailable && (
                <CardHeader className={`pb-3 ${isSongDescriptionAvailable ? 'pb-3' : 'pb-6'}`}> {/* Adjust padding if no description */}
                    <div className="flex items-center space-x-3">
            ...
            <Card className="border-0 shadow-lg">
              ...
              <CardContent className="py-4">
                ...
                <div className="bg-muted/30 p-4 rounded-md">
                  ...
                  {hasActualRomanizedLyrics && !showEnglishLyricsSectionWhenNoTabs && ( // This case would mean only Romanized is available
                    <MarkdownRenderer markdownContent={formattedRomanizedLyrics} className="lyrics-display prose-base" />
                  )}
                </div>
              </CardContent>
      ...
      {!isLoading && !error && !searchResult && isChatActive && (
         <div className="text-center text-muted-foreground py-10 h-full flex flex-col justify-center items-center">
            <LucideIcons.Music2 className="w-16 h-16 text-primary mb-4" />
            <p className="text-lg">Selected chat is empty.</p>
            <p className="text-sm">Search for lyrics or choose an item from your history.</p>
        </div>
      )}
    </div>
  );
